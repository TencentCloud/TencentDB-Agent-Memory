...
resolve_dsh_merge_helper() {
  local local_helper="${SCRIPT_DIR}/merge-dsh-config.mjs"
  local bundled_helper="${SCRIPT_DIR}/skills/setup-proxy/merge-dsh-config.mjs"
  ...
  error "找不到 dsh 配置合并脚本 merge-dsh-config.mjs"
  return 1
}
...
if ! $SCAN_FOUND; then
  ...
  if [[ -f "$_dsh_path" && -s "$_dsh_path" ]]; then
    _dsh_url=$(grep -E '^\s*baseURL:' "$_dsh_path" 2>/dev/null | head -1 | sed -E 's/.*baseURL:[[:space:]]*//; s/^"(.*)"$/\1/; s/^'\''(.*)'\''$/\1/')
    if [[ "$_dsh_url" == *"/dsh/"* ]]; then
      SCAN_PROXY=$(echo "$_dsh_url" | sed -E 's|(/dsh/.*)$||')
      SCAN_INSTANCE=$(echo "$_dsh_url" | sed -E 's|.*/dsh/([^/]+).*|\1|')
      SCAN_MODEL=$(
        awk '
          /^agent-default-model:[[:space:]]*(#.*)?$/ { in_default_model = 1; next }
          in_default_model && /^[^[:space:]#]/ { exit }
          in_default_model && /^[[:space:]]+model:[[:space:]]*/ {
            line = $0
        ...
        ' "$_dsh_path" | sed -E 's/^"(.*)"$/\1/; s/^'\''(.*)'\''$/\1/'
      )
      if [[ -f "$_dsh_cred" ]]; then
        SCAN_KEY=$(grep -E '^\s*PROXY_USER_KEY:' "$_dsh_cred" 2>/dev/null | head -1 | sed -E 's/.*PROXY_USER_KEY:[[:space:]]*//; s/^"(.*)"$/\1/; s/^'\''(.*)'\''$/\1/')
      fi
...
write_dsh() {
  ...
  local merge_helper
  merge_helper="$(resolve_dsh_merge_helper)"

  if ! command -v node &>/dev/null; then
    error "配置 dsh 需要 Node.js（dsh 本身也依赖 Node.js）"
    return 1
  fi

  TDAI_DSH_PROXY_USER_KEY="$USER_KEY" node "$merge_helper" \
