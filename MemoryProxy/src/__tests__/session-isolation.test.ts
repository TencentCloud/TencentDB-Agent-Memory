...
describe("auto-* ID 回传受生命周期约束（TTL 不能被签名绕过）", () => {
  beforeEach(() => {
    __resetAutoSessionForTests();
    resetSessionStats();
    __setAutoSessionNow(() => 1_000_000);
  ...
  it("TTL 内回传旧 ID 仍可续接（不误伤正常续轮）", () => {
    const cfg = { enabled: true, ttlMinutes: 30 } as const;
    const first = resolveOrCreateSessionId(null, "k", cfg);
    __setAutoSessionNow(() => 1_000_000 + 20 * 60_000);
    const replay = resolveOrCreateSessionId(first.sessionId, "k", cfg);
    expect(replay.sessionId).toBe(first.sessionId);
    expect(replay.autoGenerated).toBe(false);
  });

  it("空闲超过 TTL 后回传旧 ID → 不再复活，记录 expired 并生成新会话", () => {
    const cfg = { enabled: true, ttlMinutes: 30 } as const;
    const first = resolveOrCreateSessionId(null, "k", cfg);
    __setAutoSessionNow(() => 1_000_000 + 31 * 60_000);
    const replay = resolveOrCreateSessionId(first.sessionId, "k", cfg);
    expect(replay.sessionId).not.toBe(first.sessionId);
    expect(replay.autoGenerated).toBe(true);
    expect(getSessionStats().expired).toBeGreaterThan(0);
    expect(
      recentExpiredSessions().some(
        (e) => e.sid === first.sessionId && e.reason === "expired",
      ),
    ).toBe(true);
  });

  it("签名合法但活跃表无条目（重启/淘汰）→ 不复活旧 ID", () => {
    const cfg = { enabled: true, ttlMinutes: 30 } as const;
    const first = resolveOrCreateSessionId(null, "k", cfg);
    __resetAutoSessionForTests(); // 模拟重启 / 冷 pod / LRU 淘汰后
    const replay = resolveOrCreateSessionId(first.sessionId, "k", cfg);
    expect(replay.sessionId).not.toBe(first.sessionId);
    expect(replay.autoGenerated).toBe(true);
  });
