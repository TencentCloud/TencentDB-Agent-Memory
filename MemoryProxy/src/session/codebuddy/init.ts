...
async function handleSessionInitInner(
  ...
  if (state.status === "pending_team_select") {
    ...
    // ── WorkBuddy / opencode / hermes / dsh: team MORE 翻页拦截 ──
    // WB/OC/hermes 复用 CB 状态机 + 各自 form.ts 的分页 form（MORE_LABEL="更多 →"）。
    ...
    // 重渲染分支按 stage 从 codexPageIndex.teamPage 挑出，传给各 form 的
    // pageIndex）。hermes 文字模式答复先经 paginationAnswerText 抽取结构化答案。
    // dsh 客户端无 options 上限，team 不分页，即使误触也无害。
