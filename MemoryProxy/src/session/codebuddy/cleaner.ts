...
import { unwrapClarifyAnswer } from "../hermes/extractor.js";
...
export function getLastUserMessageText(messages: RawMessage[]): string {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    // 兼容五种前缀：CB 的 `call_session_init_`、WB 的 `call_wb_session_init_`
    ...
    // hermes 的 `call_hermes_session_init_`（hermes/form.ts TOOLCALL_PREFIX）。
    ...
    if (role === "tool" && tcid && /call_(wb_|dsh_|oc_|hermes_)?session_init_/.test(tcid)) {
      // hermes 的 clarify 结果是 JSON 信封（会原样回显 question / choices_offered），
      // 必须在这里解包成纯答复文本 —— 下游 CB extractor 保持零改动。
      // 其它客户端的 tool 结果不是信封，unwrapClarifyAnswer 返回 null → 原样返回。
      return unwrapClarifyAnswer(text) ?? text;
    }
