 ...
 * 隔离维度除了调用方传入的 scope（thread），还会并入 spaceId/agentSource，
 * 避免同一 API key 下不同 agent/space 复用同一 auto 会话。
 */
export function resolveOrCreateSessionId(
  ...
  const bindScope = [meta?.spaceId ?? "", meta?.agentSource ?? "", scope]
    .filter((s) => s.length > 0)
    .join("\u0001");
  if (existing && existing.length > 0) {
    if (existing.startsWith("auto-")) {
      // 签名绑定 (keyId, bindScope, fp)：跨线程/跨 agent/跨 space/跨窗口复用、
      // 旧 key 或伪造 ID
      // 一律拒绝。auto 未启用时置 rejected，调用方不得回退到 raw（防幽灵会话）。
      const fp = msgFingerprint ?? "";
      if (!verifySessionId(existing, keyId, bindScope, fp)) {
        // 拆计数：带 scope/agent/space/指纹（绑定上下文）被拒 → scopeRejected；
        // 默认上下文（无 scope/fp）被拒 → ghostRejected（签名/换 key/伪造）。
        if (bindScope || fp) record("scopeRejected");
        else record("ghostRejected");
        if (!cfg?.enabled) {
          return { sessionId: "", autoGenerated: false, rejected: true };
        }
      } else {
        // 签名合法 ≠ 一定还在生命周期内：TTL/淘汰由进程内 ACTIVE 表裁决。
        // 若客户端把 auto-* ID 当显式会话 ID 长期回传，这里必须校验内存里
        // 仍有“未过期”的活跃条目，否则旧 ID 会绕过 TTL/epoch 轮换永久续命。
        const ttlMs = (cfg?.ttlMinutes ?? 30) * 60_000;
        const now = nowFn();
        const strategy = cfg?.strategy ?? "per-key";
        if (strategy === "per-key-msg" && fp) {
          ...
          if (hitIndex >= 0) {
            ...
            if (now - hit.lastSeen < ttlMs) {
              hit.lastSeen = now;
              return { sessionId: existing, autoGenerated: false };
            }
            // 内存里还有但已过期 → 计入台账并移除，交回下方生成新会话。
            list!.splice(hitIndex, 1);
            if (list!.length === 0) ACTIVE_MSG.delete(keyId);
            record("expired");
            recordExpired({
              sid: existing,
              keyId,
          ...
          const key = bindScope ? `${keyId}\u0000${bindScope}` : keyId;
          const hit = ACTIVE.get(key);
          if (hit && hit.sid === existing) {
            if (now - hit.lastSeen < ttlMs) {
              hit.lastSeen = now;
              return { sessionId: existing, autoGenerated: false };
            }
        ...
        // 签名合法但没有活跃内存条目：可能已过期/被淘汰/重启。auto 关闭时
        // 不再复活旧 ID（防幽灵会话）；开启时落入下方生成分支（deterministic
        // 且同 epoch 时会自然收敛回同一 sid，非 deterministic 则开新会话）。
        if (!cfg?.enabled) {
          return { sessionId: "", autoGenerated: false, rejected: true };
        }
      }
  ...
  if (strategy === "per-key-msg" && msgFingerprint) {
    ...
    const sid = makeSid(keyId, bindScope, msgFingerprint, cfg, now, ttlMs);
    const entry: MsgSession = { sid, fingerprint: msgFingerprint, scope: bindScope, lastSeen: now };
  ...
  // per-key：默认一个 key 一个活跃会话；带 scope/agent/space 时按
  // (key, bindScope) 隔离，适合同一 key 下并行线程/任务/客户端互不串台的情况。
  ...
  const sid = makeSid(keyId, bindScope, "", cfg, now, ttlMs);
  ACTIVE.set(key, { sid, lastSeen: now, scope: bindScope });
