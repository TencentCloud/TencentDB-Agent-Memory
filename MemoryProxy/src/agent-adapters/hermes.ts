 ...
 *     + `model.base_url` 时请求 POST `{base_url}/chat/completions`，即
 *     **标准 OpenAI Chat Completions**，与 codebuddy / dsh / openclaw 同族。
 *   - 自定义请求头有三个等价入口（model.extra_headers /
 *     providers.<name>.extra_headers / custom_providers[].extra_headers），
 *     都会合并到 OpenAI client 的 default_headers；因此 Session Init 走
 *     header 预选路径，无需表单。
...
export const hermesAdapter: AgentAdapter & NativeChatProtocol = {
  agentKind: "hermes",
