...
export function renderKnowledgeToolsBlock(
  ...
  return [
    ...
    "起手：架构 explore；位置 search；定义 node；调用关系 callers/callees；影响面 search→impact；设计原因 wiki search→read_page。",
    "",
    "## 已绑定资源",
    resourceTags,
    "",
    ...
    "",
    "约束：tool_name 必须与 list 返回值一致；params 必须是对象（无参传 {}）；wiki 先 search 再 read_page；code=0 成功；同一调用失败 2 次后回退本地检索。",
    "</knowledge_tools>\n",
  ].join("\n");
}
