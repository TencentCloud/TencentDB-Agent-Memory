...
export function renderKnowledgeToolsBlock(
  ...
  return [
    "<knowledge_tools>",
    "团队知识路由（按顺序判断）：",
    "1. 先读本轮之前的 tool 返回。已有正文/图谱能回答时，查找阶段已完成；直接加工这些结果，不再 list/search/read。新增用户消息不会使历史结果失效。",
    "例如：已经读到文档，再要求按该文档说明理由或总结结论，应直接使用已有正文；只有明确要求更新、版本变化、资料冲突或新的缺失事实才查询。",
    "2. 否则，仅当任务确实依赖已绑定资产中的缺失事实时才调用。通用方案、本地文件足够或仅词面相关时不调用；方案中的待确认项不表示已有团队决定可查。",
    "3. 确定缺口后，按资源内容选择：",
    "- 跨文件结构、符号位置、调用/依赖或影响面：仅当 code-graph 的 match 与当前仓库一致时使用；不一致则本地检索。",
    "- 设计背景、权衡、团队定义或历史决策：按 about 选择 wiki，不要求仓库匹配。",
    "- 当前工作区的精确代码、刚修改或未提交内容：读取本地文件。",
    "起手：架构 explore；位置 search；定义 node；调用关系 callers/callees；影响面 impact（未知符号先 search）；wiki 用 search 定位、read_page 补正文，已知页面可直接读。",
    "",
    "已绑定资源：",
    resourceTags,
    "",
    "调用协议（通过 Bash 执行 curl）：",
    "URL 使用资源的 url；knowledge_id 只放 body，不拼进路径。每次请求必须带鉴权及下列上下文 headers。",
    "",
    "Step 1: 确需查询且该资源 schema 未在上下文时，先 tools/list",
    "curl -sSk -X POST <url>/tools/list \\",
    "  -H 'content-type: application/json' \\",
    ...requestHeaderLines,
    "  -d '{\"knowledge_id\":\"<知识id>\"}'",
    "",
    "返回 tools:[{name,description,params}, ...]；本会话复用。",
    "",
    "Step 2: tools/call",
    ...
    "",
    "约束：tool_name 必须与 list 返回值一致；params 必须是对象（无参传 {}）；code=0 成功；同一调用失败 2 次后回退本地检索。",
    "</knowledge_tools>\n",
  ].join("\n");
}
