...
export function renderTdaiMemoryToolsBlock(
  ...
  const lines: string[] = [
    ...
    "",
    "## Shell 兼容（必须遵守）",
    "- 先读取环境中的 Shell。若是 PowerShell，禁止直接执行下面的 Bash curl 示例，也不要调用 `curl`（它可能是 Invoke-WebRequest 别名）。",
    "- PowerShell 必须优先使用 `Invoke-RestMethod` + `ConvertTo-Json -Compress`，它能正确处理 JSON 引号、中文和 UTF-8；不要创建临时文件，不要反复探测 v1/v2 或其它端口。",
    ...
    "- Bash/zsh 才使用下方 curl 示例。HTTP 4xx 直接报告，不要换路径或端口盲目重试。",
    "",
    "覆盖范围：",
