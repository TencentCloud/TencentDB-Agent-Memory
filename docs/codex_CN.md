...
Codex 统一通过 MCP adapter 接入。只有 MCP adapter 会调用现有 Gateway，Codex adapter 不直接访问 Gateway。
...
| 变量 | 默认值 | 用途 |
|---|---|---|
| `TDAI_GATEWAY_URL` | `http://127.0.0.1:8420` | MCP adapter 使用的 Gateway 地址。 |
| `TDAI_GATEWAY_API_KEY` | 未设置 | MCP adapter 发送的 Bearer token。 |
...
- MCP recall 失败时返回 `{}`，Codex 继续处理原始 prompt。
- MCP capture 失败时只写 stderr，不阻止 Codex 结束本轮。
