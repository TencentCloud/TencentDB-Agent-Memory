...
抽取的 `enableDedup` 开启时（默认），每批新记忆先规划召回主题，再结合候选记忆的 L0 原始消息做批量审查。相比原流程最多增加一次规划调用；规划失败回退到机械混合召回，证据缺失时保留旧记忆。

每批最多规划 5 个查询；`conflictRecallTopK` 默认 5，限制在 1–10，统一候选池最多 20 条。来源只按主键读取：每条记忆最多 3 条来源、每批最多 80 条消息，每段最多 1,000 字符。SQLite、TCVDB 和 MongoDB 均支持。

单目标更新保留原 ID，`metadata.as_of` 记录来源对应的日期，`maintenance_history` 保留历史版本。合并先持久化替代记录及历史，再整合旧检索记录；L0 不变。读取和更新沿用租户、Task 和 Session 隔离。本功能完成 #1025 的 Stage 0–2；可选的定时扫描未启用，未进入有限候选池的旧记忆不会被审查。
