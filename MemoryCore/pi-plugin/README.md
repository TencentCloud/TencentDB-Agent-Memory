...
- In the TDAI panel: a **Team**, an **Agent**, and a **Task** linked to that
  agent. `x-task-id` is required for direct session registration in the current
  version — create a Task and link the Agent first (see INSTALL.md).
...
| `TDAI_TASK_ID` | **yes** | — | from the panel (Task linked to the agent); required for session init |
...
```bash
TDAI_USER_KEY=<your-user-key> TDAI_TEAM_ID=<...> TDAI_AGENT_ID=<...> TDAI_TASK_ID=<...> \
