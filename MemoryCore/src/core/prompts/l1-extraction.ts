...
2. 客观事件记忆 (type: "episodic")
   ...
   - **纠正结论例外（episodic 特别说明）**：当用户指出某个错误认知/偏差后，AI 依据该纠正（或代码证据、双方确认）给出明确的**最终纠正结论**时，该结论应提取为 episodic 记忆——这正是需要长期保留的事实，不能因"AI 输出"被整体过滤。提取句式："用户确认：[纠正后的最终事实]"；\`source_message_ids\` 需同时包含用户的纠正消息与 AI 的确认消息。**只提取被用户纠正/确认锚定的最终事实**；AI 单方面提出的新方案、假设、未经确认的分析，仍按"不应提取"处理。
...
- 一次性操作指令（如"这次、本单"相关）
- 重复的内容；AI助手自身的行为或输出（由用户纠正、确认或代码证据锚定的**最终纠正结论**除外，见上方"episodic 特别说明"）
- 不属于以上3类的信息
- 纯主观感受（不带客观事件的情绪表达）
