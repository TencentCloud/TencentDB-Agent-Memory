...
- 命名规则："我（AI）在和xxx（用户身份）做xxx（目标活动）"（**使用上述输出语言**，约 30-50 个字符或等价长度，单句，全局唯一）。xxx 中的用户姓名遵循下方通用提取原则第 4 条：只能来自用户自述，不得使用对话中提到的第三方姓名，无法确认时用"用户"。
...
2. 独立完整：记忆必须"跳出当前对话依然成立"，无上下文也能看懂。提取主体必须以"用户"或"AI"为核心。
3. 归纳合并：强关联或因果关系的多条消息，必须合并为一条完整记忆，不可碎片化。
4. 姓名归属：记忆中若出现"用户（X）"，X 只能是用户本人的明确自述（自我介绍、署名等）或系统注入的用户资料里给出的**真实姓名**。严禁把以下内容填进姓名括号：对话中第三方（同事、导师、领导、家人、朋友等）的姓名或称呼、角色标签（导师/开发者/学生…）、路径片段（如 C:\\Users\\28951）、时间状语、命理原文等任何非姓名文本。**姓名未知时括号直接省略，只写"用户"**——空括号比错名字好。无法确认时不得猜测。

【支持提取的三大类型】（必须严格遵守类型规则）
> 下面给出的"提取句式"和"触发词"仅作为中文骨架参考；**实际 \`content\` 必须按上述输出语言书写**（例如英文用户 → "The user (Maya) is a senior product manager based in Berlin"）。括号里的姓名遵循上方原则 4：英文同理，**不知道就省略括号**。

1. 个性化记忆 (type: "persona")
   - 定义：用户的稳定属性、偏好、技能、价值观、习惯（如住所、职业、饮食禁忌）。
   - 提取句式："用户喜欢/是/擅长..."；**仅当已知真实姓名**（自述或注入资料给出）时才写"用户（姓名）喜欢/是/擅长..."，否则省略括号。
   - 打分 (priority)：80-100（健康/禁忌/核心特质）；50-70（一般喜好/技能）；<50（模糊次要，可丢弃）。
   - 触发词：喜欢、习惯、经常、我这个人...

2. 客观事件记忆 (type: "episodic")
   - 定义：客观发生的动作、决定、计划或达成结果。绝不包含纯主观感受。
   - 提取句式："用户在 [最好是精确绝对时间] 于 [地点] [做了某事（可以包含起因、经过、结果）]"；**仅当已知真实姓名**时才写"用户（姓名）在..."，否则省略括号。
