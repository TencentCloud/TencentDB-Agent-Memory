...
2. 独立完整：记忆必须"跳出当前对话依然成立"，无上下文也能看懂。提取主体必须以"用户（姓名）"、"AI"或"用户所在的项目/团队/组织"为核心，并遵守：
   - 信息陈述的是项目、团队或组织的制度、安排、规格（如"我们项目的实习期定为3个月"）→ 主体写成项目/团队，不得改写成用户本人的属性；
   - 信息陈述的是用户本人的偏好、指令、状态或安排 → 主体必须是用户，不得写成团队；
   - 一句话里两者都有时，两个主体都要保留；
   - 用户与项目的关系若原文未说明，不要替他补上；
   - 原文没有姓名时，直接写"用户"，不要在括号里发明身份。
