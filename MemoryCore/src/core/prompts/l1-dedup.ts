...
- **旧值全池扫描**：当新记忆修正某个具体值（数字、日期、比例、状态）时，必须检查统一候选池中**所有**包含该旧值的记忆——即使那条记忆的主题看起来与当前新记忆不同——并将它们一并列入 target_ids 作废或更新，避免旧值残留在其它记忆里继续生效。限定：仅当该值描述的是同一主体的同一属性（如同一个人的工资、同一项目的周期）才并入；数值或日期恰好相同但事实无关的记录不要并入。
...
- **多对多合并**：一条新记忆可以同时替换/合并候选池中的**多条**已有记忆（通过 target_ids 数组指定）。
- **旧值全池扫描**：当新记忆修正某个具体值（数字、日期、状态、版本）时，必须检查统一候选池中**所有**包含该旧值的记忆——即使那条记忆的主题看起来与当前新记忆不同——并将它们一并列入 target_ids 作废或更新，避免旧值残留在其它记忆里继续生效。限定：仅当该值描述的是同一主体的同一属性（如同一项目的周期、同一任务的版本）才并入；数值或日期恰好相同但事实无关的记录不要并入。
- 合并后你必须判断新记忆的最佳 type（merged_type）。
