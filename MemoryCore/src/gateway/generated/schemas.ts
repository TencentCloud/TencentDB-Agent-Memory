 ...
 * record_id (`m_{timestamp}_{randomHex}`). Use this for user-driven memory
 * imports (markdown notes, JSON arrays, file uploads) — bypassing the
 * L0→L1 distillation pipeline. Isolation (team/agent/user) is enforced
 * by the V3 router. No dedup; callers responsible for content hygiene.
 */
export const atomicCreateRequestSchema = z.lazy(() => idFieldsSchema).and(z.object({
    "records": z.array(z.object({
        "content": z.string().max(8192),
        "type": z.optional(z.enum(["persona", "episodic", "instruction", "work_fact", "work_task", "work_method", "work_artifact"]).default("persona")),
        "scene_name": z.optional(z.string().describe("软分类命名空间,如 preferences / events / entities。不强制,可空。")),
        "priority": z.optional(z.number().min(-1).max(100).default(50)),
        "metadata": z.optional(z.record(z.string(), z.unknown()))
    })).min(1).max(500).describe("单次最多 500 条记忆。超过请分批调用。"),
    "dedup": z.optional(z.object({
        "enabled": z.boolean().default(false).describe("是否启用去重。启用后,每条记录写入前先搜索,命中则 skip/update。"),
        "threshold": z.optional(z.number().min(0).max(1).default(0.85).describe("去重阈值 0-1,score 超过则视为重复。默认 0.85。")),
        "mode": z.optional(z.enum(["skip", "update"]).default("skip").describe("命中重复时的行为:skip=跳过,update=覆盖更新。"))
    }).default({ enabled: false }))
    })) as unknown as z.ZodType<AtomicCreateRequest>

export const atomicCreateDataSchema = z.object({
    "created": z.int().describe("实际写入条数(含 update)。"),
    "ids": z.array(z.string()).describe("每条记忆的 record_id,顺序与请求 records 对齐。skip 的为空字符串。"),
    "skipped": z.optional(z.int().describe("因重复跳过的条数。")),
    "updated": z.optional(z.int().describe("因重复而更新的条数。"))
    }) as unknown as z.ZodType<AtomicCreateData>
