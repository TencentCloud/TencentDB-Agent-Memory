...
| `recall.timeoutMs` | `5000` | Recall timeout in ms; skip injection if exceeded |
| `recall.cacheOptimization.stableWrapper` | `true` | Output placeholder when no memories recalled, improving prefix-matching cache hit rate |
| `recall.cacheOptimization.splitSystemContext` | `true` | Separate stable persona/scene from dynamic content for better cache placement |
