...
const initResult = store.init({
  provider: "openai",
  model: "/home/penis/projects/granite-r2/granite-embedding-311m-multilingual-r2.Q8_0.gguf",
});
log(`init: needsReindex=${initResult.needsReindex} reason=${initResult.reason ?? "(none)"}`);
...
async function embed(text: string): Promise<Float32Array> {
  ...
  const d = (await res.json()) as Array<{ index: number; embedding: number[][] }>;
  if (!Array.isArray(d) || !d[0]?.embedding?.[0]) {
    throw new Error(`unexpected response shape: ${JSON.stringify(d).slice(0, 200)}`);
  }
...
process.stderr.write(`done: l1Count=${result.l1Count}, l0Count=${result.l0Count}, elapsed=${elapsed.toFixed(1)}s\n`);
