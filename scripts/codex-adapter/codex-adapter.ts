 ...
 * and the binary delegates all Gateway HTTP details to the shared
 * GatewayMemoryClient/createGatewayPlatformAdapter boundary.
 */
...
import { GatewayMemoryClient, createGatewayPlatformAdapter } from "../../src/adapters/gateway-client/index.js";
...
async function main(): Promise<void> {
  ...
  const client = new GatewayMemoryClient({ baseUrl: opts.gatewayUrl, apiKey: opts.apiKey });
  const memory = createGatewayPlatformAdapter({
    client,
    platform: "codex",
    resolveContext: () => ({ sessionKey: opts.sessionKey, sessionId: opts.sessionId }),
  });
  ...
  switch (opts.command) {
    case "recall": {
      ...
      const result = await memory.prefetch(query);
    ...
    case "capture": {
      ...
      const result = await memory.captureTurn({
        userText: user,
    ...
    case "search": {
      ...
      const result = await memory.searchMemories({ query, limit: opts.limit, type: opts.type, scene: opts.scene });
    ...
    case "conversation-search": {
      ...
      const result = await memory.searchConversations({ query, limit: opts.limit });
      writeOutput(opts, opts.json ? JSON.stringify(result, null, 2) : (result.results ?? ""));
      break;
    }
    default:
