...
import { resolveRoleDirForRead } from "../src/gateway/role-paths.js";
...
async function main(): Promise<void> {
  ...
  // Роли живут под КОРНЕМ ДАННЫХ, не под $HOME: tz-07 критерий 2. Прежний
  // resolveRoleDir(home) давал <home>/roles и мимо конфигурационного корня.
  const roleDir = resolveRoleDirForRead(dataDir);
  const sharedScratchRoot = config.data.scratchRoot;
  ...
  console.log(
    "memory-keeper trigger:",
    mk.status,
    mk.accepted ? "accepted ✓" : "refused",
  );
  if (!mk.accepted) {
  ...
  console.log(
    "same-role memory-keeper #2:",
    mk2.status,
    mk2.accepted ? "accepted (BAD)" : "busy ✓",
  );
  if (mk2.accepted) {
    console.error(
      "FAIL: same-role second trigger was accepted (single-flight broken)",
    );
    process.exit(1);
  }

  // Different role (night-keeper) → must be ACCEPTED in parallel.
  const nk = await orch.trigger({
    reason: "parallel-smoke-night",
    runType: "night-keeper",
  });
  console.log(
    "night-keeper trigger:",
    nk.status,
    nk.accepted ? "accepted ✓" : "refused",
  );
  if (!nk.accepted) {
    console.error(
      "FAIL: night-keeper refused while memory-keeper in flight (per-role lock broken)",
    );
    process.exit(1);
  }
  ...
  await orch.stop();
  console.log(
    "PARALLELISM OK: memory-keeper + night-keeper ran concurrently, same-role busy honored",
  );
}
