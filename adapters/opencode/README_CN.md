...
插件对 OpenCode 重复 idle、共享同一状态目录的并发 OpenCode 进程、进程重启和单通道失败提供**本地持久化去重**。Gateway 当前的 conversation add 接口没有客户端幂等键，因此存在一个无法在客户端彻底消除的窄窗口：Gateway 已写入，但响应在客户端收到前丢失，此时重试可能产生服务端重复。这里不宣称端到端 exactly-once；若 Gateway 后续支持 idempotency key，可直接使用本插件的稳定 turn key 补齐该语义。
