...
- ``POST /search/conversations``(session-scoped conversation search tool)
...
@dataclass
class RecallResult:
    """Response fields of POST /recall that the adapter consumes."""
...
@dataclass
class SearchResult:
    """Normalized search results returned by the search tools."""
...
class MemoryGatewayClient:
    ...
    async def health(self) -> HealthStatus:
        headers: dict[str, str] = {}
        try:
            response = await self._client.get(f"{self._base_url}/health", headers=headers)
        except httpx.HTTPError as exc:
    ...
    async def recall(self, query: str, session_key: str, user_id: str = "") -> RecallResult:
        payload: dict[str, Any] = {"query": query, "session_key": session_key}
        if user_id:
            payload["user_id"] = user_id
        data = await self._post("/recall", payload)
        return RecallResult(
            context=data.get("context", ""),
            prepend_context=data.get("prepend_context", ""),
