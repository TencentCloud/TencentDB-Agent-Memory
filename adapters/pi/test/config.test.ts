...
describe("loadConfig", () => {
  ...
  it("rejects endpoint credentials and header-injection characters", () => {
    const credentialed = loadConfig({
      ...validEnv,
      TDAI_MEMORY_ENDPOINT: "https://user:password@memory.example.com",
    });
    expect(credentialed.ok).toBe(false);
    if (!credentialed.ok) {
      expect(credentialed.errors.join(" ")).toContain("must not include username or password");
    }

    const injected = loadConfig({
      ...validEnv,
      TDAI_MEMORY_SERVICE_ID: "service-1\r\nX-Leaked: yes",
    });
    expect(injected.ok).toBe(false);
    if (!injected.ok) {
      expect(injected.errors.join(" ")).toContain("control characters");
    }

    const withQuery = loadConfig({
      ...validEnv,
      TDAI_MEMORY_ENDPOINT: "https://memory.example.com/?tenant=other",
    });
    expect(withQuery.ok).toBe(false);
    if (!withQuery.ok) {
      expect(withQuery.errors.join(" ")).toContain("query or fragment");
    }
  });
