...
当前 P0 尚未完成：outbox 手动 flush 命令、setup 向导、完整 Skill 双管线与安全同步、adapter CI，以及完整故障矩阵。outbox 已具备跨进程原子 lease、持久化退避与 `.dead` 隔离；非空 L0/L1/L2/L3 的真实模型生成与 Pi hook 注入已有受管 E2E 脚本验证；它不等于本节其余故障场景已完成。下文章节描述最终目标；未完成项不能仅凭本文描述视为已经交付。
