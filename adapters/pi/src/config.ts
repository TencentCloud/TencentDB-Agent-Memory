...
export function loadConfig(env: Environment = process.env): ConfigResult {
  ...
  for (const key of REQUIRED_KEYS) {
    ...
    else if (CONTROL_CHARACTERS.test(env[key]!.trim())) {
      errors.push(key + " must not contain control characters");
    }
  }
  if (env.TDAI_MEMORY_TASK_ID?.trim() && CONTROL_CHARACTERS.test(env.TDAI_MEMORY_TASK_ID.trim())) {
    errors.push("TDAI_MEMORY_TASK_ID must not contain control characters");
  }
  ...
  try {
    ...
    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
      ...
      errors.push("TDAI_MEMORY_ENDPOINT must not include username or password");
    } else if (parsed.search || parsed.hash) {
      errors.push("TDAI_MEMORY_ENDPOINT must not include a query or fragment");
