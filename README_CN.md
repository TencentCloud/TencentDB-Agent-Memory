...
| `recall.injectionMode` | `"epoch"` | `epoch` 仅把召回工作集变化写成隐藏的追加事件；`prepend` / `append` 保留旧的逐轮临时注入 |
| `recall.epochMaxTokens` | `8192` | Memory Epoch 持久事件硬上限；实际预算同时不超过 OpenClaw 上下文预算的 10%，到顶后转为临时召回，compaction 后重新开放 |
