...
【严格时间戳与元数据规则】
1. 顶部元数据（必填，必须是 Mermaid 注释，不能使用 %%{...}%% directive）：%% mmd-meta: {"taskGoal":"一句话总结此次任务的目标（可动态更新）","progress":90,"createdTime":"ISO时间","updatedTime":"ISO时间"}（progress 按实际进度填写 0-100，updatedTime 为 node 中的最新时间）。
