...
describe("resolveAfterToolCallMessages", () => {
  it("uses non-empty event messages when present", () => {
  ...
  it("prefers a non-empty fallback over an empty event messages array", () => {
    const historyMessages = [{ role: "user", content: "from history" }];

    expect(resolveAfterToolCallMessages({ messages: [] }, { historyMessages })).toEqual({
