...
import { TAG, type RecallDiagnostic, type RecallItem, type StrategyResult } from "./types.js";
  ...
  if (vectorStore?.isFtsAvailable()) {
    ...
    if (ftsQuery) {
      ...
      if (ftsResults.length > 0) {
        ...
        const filtered = decayed.filter((i) => i.score.final >= threshold).slice(0, maxResults);
        ...
        if (ftsResults.length <= maxResults) {
          ...
          const rescued = ftsResults.slice(0, maxResults).map((r) => toItem(r, projectId, scopeDecayCfg, ["below-threshold-small-set"]));
