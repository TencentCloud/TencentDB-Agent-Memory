...
async function performAutoRecallInner(params: {
  ...
  if (!userText || userText.length === 0) {
    ...
    // Items and their lines travel together through the budget, so the
    // reported memories keep the id and the real score the store gave them
    // (before tz-10a they were regex-parsed back out of the rendered line
    // with score 0 — tz-10 C10.3).
    ...
    if (recallDiagnostics.length > 0) {
      logger?.debug?.(
        `${TAG} Recall diagnostics: ` +
          recallDiagnostics.map((d) => `${d.stage}:${d.code}`).join(", "),
      );
    }
  ...
  if (memoryLines.length === 0 && !personaContent && !sceneNavigation) {
    ...
    // and an empty memory are different answers (tz-10 C10.5). No context
    // fields are set, so no caller injects anything either way.
  ...
  const appendSystemContext =
    stableParts.length > 0 ? stableParts.join("\n\n") : undefined;
