...
async function performAutoRecallInner(params: {
  ...
  // Search relevant memories (L1 layer) only in explicit auto mode.  The
  // zero-config D-plan path is tool-only: stable context is appended to the
  // system prompt and dynamic recall is retrieved through tools on demand.
  ...
  if (cfg.recall.mode === "auto" && cfg.recall.showInjected && memoryLines.length > 0) {
    prependContext =
