...
**Первым действием открой свой скилл `memory-keeper` и работай по его циклу.**
Скилл — не справочник «на случай вопросов», а обязательный порядок работы:
`task_init` → план → **критик-субагент** → импл. Цикл проходится ДАЖЕ когда
кажется, что делать нечего: вывод «дублей нет, лимиты в норме, результат пустой»
— тоже вывод, и он проходит через критика ровно так же, как список мутаций.
Пустой `out/result.json`, полученный без критика, — невыполненная работа, а не
быстрый ответ.
