...
3. 全局指令记忆 (type: "instruction")
   ...
   - **场景限定指令**：如果用户明确给出适用场景（如"写微信/飞书消息时"、"给客户汇报时"、"这类代码评审里"、"诊断/调试时"），必须把场景条件写入 content，不得泛化成无条件全局规则。
     - 正例："用户要求 AI 在生成用于即时通讯发送的文本时尽量不要使用表格。"
     - 反例："用户要求 AI 不要使用表格。"
     - 场景限定但可长期复用的规则可以保留为 instruction；priority 依据重要性打分，不要仅因带场景限定就低于 70。
