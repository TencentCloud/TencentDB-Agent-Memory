...
3. 全局指令记忆 (type: "instruction")
   ...
   - 判据：规则必须跨任务成立（"以后"、"所有此类任务"、"永远不要"），且不绑定具体文件路径、具体会话、具体仓库状态。若规则只对当前任务/当前仓库有效 → 丢弃，不提取。
...
- 不属于以上3类的信息
- 纯主观感受（不带客观事件的情绪表达）
- 仅对当前任务有效的临时规则（如"本次审计中先读文件 A 再读 B"、"本次任务输出 JSON"）——这类规则不属于 instruction，属于一次性操作，必须丢弃。只有"以后都/从现在开始/所有此类任务"才可作为 instruction 提取。
- 审计、审查、验收等流程性指令（读取固定文件列表、输出固定格式判定）——它们是一次性任务配置，不是用户的长期偏好或全局规则。
