...
2. **判断是否同一事实/事件**：主体相同、主题一致、时间接近、scene_name 相似
   - 对 instruction 记忆，必须同时比较适用场景/条件。规则文本相似但适用场景不同（例如"即时通讯文本不用表格" vs "正式报告可用表格"）不是同一条无条件全局规则。
   - merge / update instruction 时，merged_content 必须保留所有仍然成立的场景限定词；不得把带条件的规则改写成无条件全局规则。
   - instruction 的 merged_priority 不得因为 merge 自动升高；只有用户明确多次表达为全局长期规则时才可提高。不得把诊断、调试、单次任务中的指令提升为核心全局规则。
