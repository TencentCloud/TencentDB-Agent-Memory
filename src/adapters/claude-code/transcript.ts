 ...
 *
 * The Stop hook captures one turn as prompt + final assistant message. Everything
 * in between (tool calls, tool results, intermediate assistant text) never reaches
 * L0 that way. At SessionEnd this module reads the session transcript (the JSONL
 * Claude Code writes under its projects directory), normalises the delta since the
 * last SessionEnd into L0 messages, and leaves the turns Stop already captured out
 * so nothing lands twice. The Gateway records what it is sent; it does not
 * deduplicate by message id, so exclusion has to happen here.
 */
...
export interface NormalizeTranscriptOptions {
  sessionId: string;
  /** Prompt ids whose prompt + final assistant text the Stop hook already captured. */
  capturedPromptIds?: Set<string>;
  /** Longest content per L0 message; longer text is split into numbered chunks. */
  chunkChars?: number;
  /** Tool inputs and results are quoted up to this many characters. */
...
/**
 * Credential shapes that tool traffic routinely carries (env dumps, config files,
 * curl commands) and that have no place in shared memory. Each match is replaced
 * by `[redacted:<kind>]`; the surrounding text stays so the event is still legible.
 */
...
/** Replace credential-shaped substrings; the `value` pattern keeps its key so `API_KEY=` stays readable. */
...
/** Parse a transcript JSONL file into the entries that carry conversation content. */
...
/** Entries after the one whose uuid is `afterUuid`; all entries when it is unknown. */
...
/** Prompt ids of the user prompts in `entries`, in order of first appearance. */
...
interface Part {
  text: string;
  /** Plain assistant prose, as opposed to a quoted tool call. */
  prose: boolean;
}

/**
 * Normalise transcript entries into L0 messages.
 *
 * Policy (mirrors the pi extension): thinking is dropped; a tool call becomes
 * assistant text `[tool_use ...]`; a tool result becomes user text `[tool_result ...]`;
 * images are dropped; long content is chunked. For a turn whose prompt id is in
 * `capturedPromptIds`, the prompt and the final assistant prose are left out because
 * the Stop hook already sent them; that turn's tool calls and results stay.
 * Credential-shaped substrings are redacted from every message (see `redactSecrets`).
 */
export function normalizeTranscript(entries: TranscriptEntry[], options: NormalizeTranscriptOptions): TranscriptMessage[] {
  ...
  // The final assistant prose of a captured turn is what Stop sent as
  // last_assistant_message: the last assistant draft with prose before the next prompt.
...
/**
 * Where the transcript lives. Claude Code passes `transcript_path` in every hook
 * payload; when it is absent the file is looked up under the projects directory,
 * whose per-project folder is the cwd with every non-alphanumeric character
 * replaced by `-`.
 */
