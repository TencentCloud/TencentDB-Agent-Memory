...
vi.mock("../core/record/l1-writer.js", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../core/record/l1-writer.js")>();
...
describe("ApplyExecutor", () => {
  ...
  it("deleteL1 id not presented to the keeper → 400, nothing applied", async () => {
    ...
    const r = await executor().apply(
      body(
        { deleteL1: [{ id: "m_x", updatedAt: "2026-08-01T00:00:00Z" }] },
        {},
        [],
      ),
    );
  ...
  it("merge target outside its cluster → 400", async () => {
    ...
    const r = await executor().apply(
      body(
        {
          merge: [
            { cluster: ["m_a", "m_b"], target: "m_zzz", content: "merged" },
          ],
        },
        {},
        ["m_a", "m_b"],
  ...
  it("merge cluster member not presented to the keeper → 400", async () => {
    ...
    const r = await executor().apply(
      body(
        {
          merge: [
            { cluster: ["m_a", "m_b"], target: "m_a", content: "merged" },
          ],
        },
        {},
        ["m_a"],
  ...
  it("rewriteBlock path outside allowlist → 400", async () => {
    const r = await executor().apply(
      body(
        {
          rewriteBlock: [{ path: "../../etc/passwd", content: SCENE_CONTENT }],
        },
        {},
        [],
      ),
  ...
  it("rewriteBlock with a Cyrillic scene path applies (Unicode allowlist, not ASCII-only)", async () => {
    ...
    const r = await executor().apply(
      body(
        { rewriteBlock: [{ path: cyrPath, content: newContent }] },
        baseline(dataDir, [cyrPath]),
        [],
      ),
    );
    expect(r.ok, JSON.stringify(r)).toBe(true);
    expect(r.error).toBeUndefined();
    expect(fs.readFileSync(path.join(dataDir, cyrPath), "utf-8")).toBe(
      newContent,
    );
  });
  ...
  it("rewriteBlock content without META delimiters → 400 before any mutation", async () => {
    ...
    const r = await executor().apply(
      body(
        { rewriteBlock: [{ path: scenePath, content: "body without meta" }] },
        baseline(dataDir, [scenePath]),
        [],
      ),
    );
    expect(r.statusCode).toBe(400);
    ...
    // file untouched
    expect(fs.readFileSync(path.join(dataDir, scenePath), "utf-8")).toBe(
      SCENE_CONTENT,
    );
  });
  ...
  it("rewriteBlock content over the scene limit → 400 (zod)", async () => {
    ...
    const r = await executor().apply(
      body(
        { rewriteBlock: [{ path: scenePath, content: over }] },
        baseline(dataDir, [scenePath]),
        [],
      ),
    );
  ...
  it("rewritePersona over the persona limit → 400 (zod)", async () => {
    ...
    const r = await executor().apply(
      body(
        { rewritePersona: "y".repeat(PERSONA_LIMIT_CHARS + 5) },
        baseline(dataDir, ["persona.md"]),
        [],
      ),
    );
    expect(r.statusCode).toBe(400);
  ...
  it("rewriteBlock path not covered by the manifest baseline → 400", async () => {
    const r = await executor().apply(
      body(
        {
          rewriteBlock: [
            { path: "scene_blocks/_global/ok.md", content: SCENE_CONTENT },
          ],
        },
        {},
        [],
  ...
  it("deleteL1Batch=false → aborted 500, nothing applied, record intact", async () => {
    ...
    const r = await executor({ vectorStore: failingStore }).apply(
      body(
        { deleteL1: [{ id: "m_del", updatedAt: "2026-08-01T00:00:00Z" }] },
        {},
        ["m_del"],
      ),
    );
  ...
  it("stale delete (updatedAt drift) → aborted 409, record intact", async () => {
    ...
    const r = await executor().apply(
      body(
        { deleteL1: [{ id: "m_fresh", updatedAt: "2026-07-01T00:00:00Z" }] },
        {},
        ["m_fresh"],
      ),
    );
  ...
  it("heal: abort on 2nd mutation, re-run skips applied and completes the rest", async () => {
    ...
    const r1 = await executor().apply(
      body(diff, manifest, ["m_a", "m_b", "m_c"]),
    );
    ...
    expect(fs.readFileSync(path.join(dataDir, "persona.md"), "utf-8")).toBe(
      "new persona",
    );
  });
  ...
  it("manifest drift (file touched outside apply) → aborted 409, nothing applied", async () => {
    ...
    const r = await executor().apply(
      body(
        { rewriteBlock: [{ path: scenePath, content: newContent }] },
        manifest,
        [],
      ),
    );
    ...
    expect(fs.readFileSync(path.join(dataDir, scenePath), "utf-8")).toContain(
      "drifted",
    );
  });
  ...
  it("delete of an already-missing record → skipped, not aborted", async () => {
    const r = await executor().apply(
      body(
        { deleteL1: [{ id: "ghost", updatedAt: "2026-08-01T00:00:00Z" }] },
        {},
        ["ghost"],
      ),
    );
  ...
  it("criterion 5 — dedup via deleteL1 does NOT trigger a full reindex", async () => {
    ...
    const r = await executor().apply(
      body(
        { deleteL1: [{ id: "d_2", updatedAt: "2026-08-01T00:00:00Z" }] },
        {},
        ["d_2"],
      ),
    );
  ...
  it("criterion 3 — after apply, scene/persona files on disk are within limits", async () => {
    ...
    const r = await executor().apply(
      body(
        {
          rewriteBlock: [{ path: scenePath, content: newScene }],
          rewritePersona: newPersona,
        },
        manifest,
        [],
      ),
    ...
    const onDiskPersona = fs.readFileSync(
      path.join(dataDir, "persona.md"),
      "utf-8",
    );
  ...
  it("livelock cap: persistent mismatch survives 2 reindexAll retries, per-row backfill resolves", async () => {
    ...
    const stub = {
      ...
      reindexL0Records: async () => {
        throw new Error("L0 heal must not run when L1 resolved");
      },
    } as unknown as IMemoryStore;
  ...
  it("livelock cap: backfill also unresolved → run failed, cap documented in the error", async () => {
    ...
    expect(r.error).toMatch(
      new RegExp(
        `unresolved after ${MAX_REINDEX_RETRIES} reindex attempt\\(s\\) \\+ per-row backfill`,
      ),
    );
  });
  ...
  it("rewriteBlock: atomic content write, backup created, scene index rebuilt", async () => {
    ...
    const backups = fs
      ...
      .filter((f) => f.startsWith("apply-") && f.endsWith(".bak"));
    ...
    const index = JSON.parse(
      fs.readFileSync(
        path.join(dataDir, ".metadata", "scene_index", "_global.json"),
        "utf-8",
      ),
    );
  ...
  it("syncSceneIndex failure → run failed (files stay applied, heal possible)", async () => {
    ...
    fs.writeFileSync(
      path.join(dataDir, ".metadata", "scene_index"),
      "not a dir",
      "utf-8",
    );
    const manifest = baseline(dataDir, [scenePath]);
  ...
  // ============================
  // rewriteRecord — in-place record rewrite (created_time preserved)
  // ============================

  it("rewriteRecord: content rewritten, created_time preserved, vec-vs-meta consistent", async () => {
    seedRecord("m_rw", "old content", "2026-08-01T00:00:00Z");
    const r = await executor().apply(
      body(
        {
          rewriteRecord: [
    ...
    expect(r.ok).toBe(true);
    expect(r.applied.rewrites).toEqual(["m_rw"]);

    const rows = await store.queryL1Records();
    expect(rows.length).toBe(1);
    expect(rows[0]?.record_id).toBe("m_rw");
    expect(rows[0]?.content).toBe("new rewritten content");
    // created_time preserved (age must not reset)
    expect(rows[0]?.created_time).toBe("2026-08-01T00:00:00Z");
  });

  it("rewriteRecord: not presented → 400", async () => {
    seedRecord("m_rw2", "old", "2026-08-01T00:00:00Z");
    const r = await executor().apply(
      body(
        {
          rewriteRecord: [
            { id: "m_rw2", updatedAt: "2026-08-01T00:00:00Z", content: "x" },
          ],
        },
        {},
        [],
  ...
  it("rewriteRecord: stale updatedAt drift → 409, record intact", async () => {
    seedRecord("m_rw3", "old", "2026-08-01T00:00:00Z");
    const r = await executor().apply(
      body(
        {
          rewriteRecord: [
            { id: "m_rw3", updatedAt: "2026-07-01T00:00:00Z", content: "x" },
          ],
        },
        {},
        ["m_rw3"],
    ...
    expect(r.statusCode).toBe(409);
    expect(r.error).toMatch(/was updated since the diff/);
    expect(store.countL1()).toBe(1);
  });

  it("rewriteRecord: heal-skip when content already equals diff → skipped.rewrites", async () => {
    seedRecord("m_rw4", "same content", "2026-08-01T00:00:00Z");
    const r = await executor().apply(
      body(
        {
          rewriteRecord: [
    ...
    expect(r.ok).toBe(true);
    expect(r.skipped.rewrites).toEqual(["m_rw4"]);
  ...
  it("rewriteRecord: id overlap with merge in same diff → 400 (intersection ban)", async () => {
      ...
      body(
        {
          merge: [
            {
              cluster: ["m_rw5a", "m_rw5b"],
              target: "m_rw5a",
              content: "merged",
            },
          ],
          rewriteRecord: [
            { id: "m_rw5a", updatedAt: "2026-08-01T00:00:00Z", content: "x" },
          ],
        },
        {},
        ["m_rw5a", "m_rw5b"],
    ...
    expect(store.countL1()).toBe(2); // nothing mutated
  });

  it("rewriteRecord: content over L1 cap → 400", async () => {
    seedRecord("m_rw6", "old", "2026-08-01T00:00:00Z");
    const big = "x".repeat(601);
    const r = await executor().apply(
      body(
        {
          rewriteRecord: [
            { id: "m_rw6", updatedAt: "2026-08-01T00:00:00Z", content: big },
          ],
        },
        {},
        ["m_rw6"],
  ...
  it("merge with missing target → skip (not abort), day-path regression", async () => {
    // target m_gone never seeded; member m_live exists
    seedRecord("m_live", "live", "2026-08-01T00:00:00Z");
    const r = await executor().apply(
      body(
        {
          merge: [
            {
              cluster: ["m_gone", "m_live"],
              target: "m_gone",
              content: "merged",
            },
          ],
    ...
    expect(r.ok).toBe(true);
    expect(r.skipped.merges).toEqual(["m_gone"]);
    expect(r.applied.merges).toEqual([]);
    expect(store.countL1()).toBe(1); // member untouched
  });
});
...
describe("handleMemoryApply (HTTP)", () => {
  ...
  function ctx(): {
    ...
    return {
      core: {
        ...
        getEmbeddingService: () => fakeEmbedding,
      } as never,
  ...
  it("invalid diff → 400 with structured result", async () => {
    ...
    await handleMemoryApply(
      ctx() as never,
      makeReq(JSON.stringify({ wrong: 1 })),
      res,
    );
    expect(status()).toBe(400);
