...
- **Auto-recall prompt cache 分区** ([#120](https://github.com/TencentCloud/TencentDB-Agent-Memory/issues/120))：稳定的 Persona / Scene Navigation / Tools Guide 改用 `prependSystemContext`，进入 OpenClaw `CACHE_BOUNDARY` 之前；随查询变化的 L1 召回改用 `appendSystemContext`，不再通过 `prependContext` 写入用户消息。该结构避免 `showInjected` 路径每轮持久化一份召回块，同时在连续轮次召回结果相同时保持完整请求前缀一致。Gateway / Hermes 会按稳定段、动态段顺序合并，保持原有单字符串协议兼容。
