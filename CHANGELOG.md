...
#### 数据一致性
- **Checkpoint 计数器只增不减，cleanup 后与真实数据漂移** ([#157](https://github.com/TencentCloud/TencentDB-Agent-Memory/issues/157))：`recall_checkpoint.json` 中的 `total_memories_extracted` 和 `l0_conversations_count` 仅在 L0 capture / L1 完成时递增，`memory-cleaner` 清理 JSONL 分片或 SQLite 过期行、以及手工编辑 checkpoint 后从不下调；结果计数长期偏高，导致 `memories_since_last_persona` / `total_processed` 触发 L3 persona 生成阈值时机错乱，状态查询也持续高报。新增 `CheckpointManager.recalibrate(dataDir, store?)`：JSONL (`records/*.jsonl`) 为 L1 数据源、可选 store probe (`countL0` / `countL1`) 为 L0 数据源；`MAX(jsonl, dbL1)` 取容忍分片未同步；**只下调不上调**（避免 crash 前未 flush 的写入落盘后被双计）；对 `memories_since_last_persona` 做 clamp 防止 persona 幻触发。`TdaiCore.initialize()` 中在 `storeReady` 后 fire-and-forget 调用一次，best-effort，永不阻塞启动。11 组 vitest 覆盖 downward 修正、no-grow 保护、probe 抛错回退、malformed JSONL 跳过、并发写入锁。

#### 数据安全 / 数据隔离
- **`buildFtsQuery` 未剥离 FTS5 保留操作符，用户输入可改变查询语义** ([#160](https://github.com/TencentCloud/TencentDB-Agent-Memory/issues/160))：`src/core/store/sqlite.ts` 中的 `buildFtsQuery()` 将用户输入分词后逐 token 双引号包裹并以 `OR` 连接，虽然缓解了大部分注入，但从未剥离 FTS5 保留关键字 `AND` / `OR` / `NOT` / `NEAR`。按 SQLite FTS5 语法 §3，这四个关键字**大小写敏感**——只有完全大写形式才是操作符，小写 / 混合大小写属于普通 bareword。修复：(1) 新增 `sanitizeFtsInput()` 在分词前用 `\b(?:AND|OR|NOT|NEAR)\b` （区分大小写）替换为空格；(2) 在 jieba 与 fallback 两条路径各加一道 UPPERCASE 关键字过滤，防御未来分词器变更后 re-emit 出裸 `AND`；(3) 保留小写 `and` / `or` / `not` / `near` 以及 `ANDROID` / `NEARBY` 等仅含关键字子串的词，避免误伤中文混合语料的召回。`sqlite.buildFtsQuery.test.ts` 新增 18 个用例覆盖 sanitizer、fallback 路径、jieba 路径（含 `NEAR(...)` group syntax 中和、大小写敏感、子串保留、双引号剥离不变式）。
